...
import { GAConnect, GAParams } from '../../../../../API/Model/Connects/GAModel';
import BasicColumns from '../Charts/BasicColumns/BasicColumns';
...
const GAConnectWidget: FC = () => {
  const activeInstance = useActiveInstanceStore((state) => state.instance);
  const getGAConnects = useGetGAConnect(activeInstance!.id);
  const [choosenDimension, setChoosenDimension] = useState<string>('');
  const [gaData, setGAData] = useState<TransformedRow[]>([]);
  ...
  const parseChatMessageForGAParameters = async (
    ...
    try {
      ...
      if (!response || !response.choices || response.choices.length === 0) {
        console.error('OpenAI response is empty or malformed');
      }
      ...
      setChoosenDimension(parsedResponse.dimensions[0]);
  ...
  const queryGoogleAnalytics = async (
    ...
    try {
      ...
      setGAData(transformDataset(gaResponse.data));
    } catch (error) {
  ...
  const transformDataset: (data: Data) => TransformedRow[] = (data) => {
    if (!data.rows || !Array.isArray(data.rows)) {
      queryGoogleAnalytics(
        getGAConnects,
        'Pick 3-5 random metrics and 1-2 dimensions from the list and provide me data for past 30 days'
      );
      return [];
    }
    ...
    try {
      return data.rows.map((row) => {
        ...
        row.dimensionValues.forEach((dimension, index) => {
          ...
          if (headerName) {
            transformed.dimensions[headerName] = dimension.value;
          }
        });
        ...
        row.metricValues.forEach((metric, index) => {
          ...
          if (headerName) {
            transformed.metrics[headerName] = metric.value;
          }
        });
  ...
  useEffect(() => {
    if (!getGAConnects) return;
    queryGoogleAnalytics(
      getGAConnects,
      'Pick 3-5 random metrics and 1 dimensions from the list and provide me data for past 30 days'
    );
    ...
    <Box display="flex">
      {getGAConnects ? (
        <BasicColumns
          ...
          description="The chart below was created using artificial intelligence to provide deeper insights into your Google Analytics Metrics & Dimensions. (30 Days Focus)"
        />
      ) : (
        <Box>
          <Text>Google Analytics Account not Connected</Text>
        </Box>
      )}
    </Box>
  );
...
export default GAConnectWidget;
